  ...
  const render = (k) => {
    ...
    $("fc-gb").innerHTML = gb.slice(0, 10).map((p, idx) => `
      <div class="bar-row${gbDecided && idx === 0 ? " gb-winner" : ""}">
        <div class="who">${gbDecided && idx === 0 ? "🏆 " : ""}${flag(p.team)}${p.player}${p.e != null ? `<span class="gb-goals">${Math.abs(p.e - (p.g ?? 0)) < 0.05 ? `<b>${p.g ?? 0}</b> goals` : `<b>${p.g ?? 0}</b> scored · <b>${p.e.toFixed(1)}</b> expected at the end`}</span>` : ""}</div>
